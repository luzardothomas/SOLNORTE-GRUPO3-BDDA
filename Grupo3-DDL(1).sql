...
CREATE TABLE socios.rolVigente (
	...
	estadoLogicoRolVigente BIT NOT NULL CONSTRAINT estadoVigencia DEFAULT (1)
);
GO
...
CREATE TABLE pagos.facturaEmitida (
	...
	modalidadCobro VARCHAR(25) NOT NULL,
	importeBruto DECIMAL(10, 2) NOT NULL CHECK (subtotalcuota >= 0),
	...
	estadoFactura varchar(10) NOT NULL CHECK (estadoFactura IN('Pendiente' OR 'Pagada')),
...
CREATE TABLE pagos.reembolso (
    ...
    FOREIGN KEY (idCobroOriginal) REFERENCES pagos.cobroFactura(idCobro),
...
CREATE FUNCTION socios.validarCUIL(@cuil VARCHAR(13))
RETURNS BIT
AS
BEGIN
...
CREATE FUNCTION socios.validarDNI(@dni VARCHAR(15))
RETURNS BIT
AS
BEGIN
...
--Stored Procedures: Inserción / Modificación / Borrado 
...
EXEC socios.sp_InsertarCategoriaSocio @tipo = 'Cadete', @costoMembresia = 25000.00;

-- 2.2 inserción con costo inválido (negativo) -> falla por CHECK de tabla
BEGIN TRY
    EXEC socios.sp_InsertarCategoriaSocio @tipo = 'Mayor', @costoMembresia = -50.00;
END TRY
BEGIN CATCH
...
BEGIN TRY
    EXEC socios.sp_InsertarCategoriaSocio @tipo = '', @costoMembresia = 50.00;
END TRY
BEGIN CATCH
    PRINT 'Error esperado: ' + ERROR_MESSAGE();
END CATCH;
...
BEGIN
  SET NOCOUNT ON;
  IF socios.fn_ValidarDNI(@dni) = 0
  BEGIN
    RAISERROR('DNI inválido.',16,1);
    RETURN;
  END
  IF socios.fn_ValidarCUIL(@cuil) = 0
  BEGIN
...
BEGIN
  ...
  IF @dni IS NOT NULL AND socios.fn_ValidarDNI(@dni) = 0
  BEGIN
    RAISERROR('DNI inválido.',16,1);
    RETURN;
  END
  IF @cuil IS NOT NULL AND socios.fn_ValidarCUIL(@cuil) = 0
  BEGIN
    RAISERROR('CUIL inválido.',16,1);
    RETURN;
  END
...
CREATE PROCEDURE socios.insertarRolDisponible
    @idRol       INT,
...
BEGIN
    ...
    BEGIN
        ...
        RETURN;
    END
    IF EXISTS (SELECT 1 FROM socios.rolDisponible WHERE idRol = @idRol AND activo = 1)
    BEGIN
...
CREATE PROCEDURE socios.actualizarRolDisponible
    @idRol       INT,
    @descripcion VARCHAR(50)
AS
BEGIN
    ...
    -- Verificar existencia
    IF NOT EXISTS (SELECT 1 FROM socios.rolDisponible WHERE idRol = @idRol AND activo = 1)
    BEGIN
...
CREATE PROCEDURE socios.desactivarRolDisponible
    @idRol INT
AS
BEGIN
    SET NOCOUNT ON;
    ...
    -- Verificar existencia
    IF NOT EXISTS (SELECT 1 FROM socios.rolDisponible WHERE idRol = @idRol AND  = 1)
    BEGIN
        RAISERROR('No existe un rol activo con id %d.',16,1,@idRol);
        RETURN;
    END
...
CREATE PROCEDURE socios.actualizarGrupoFamiliar
  @idGrupoFamiliar       INT,
  @cantidadGrupoFamiliar INT
AS
BEGIN
  ...
  -- Verificar existencia
  IF NOT EXISTS (SELECT 1 FROM socios.grupoFamiliar WHERE igGrupoFamiliar = @idGrupoFamiliar)
  BEGIN
  ...
  WHERE igGrupoFamiliar = @idGrupoFamiliar;
END
GO
...
IF OBJECT_ID('actividades.insertarDeporteDisponible', 'P') IS NOT NULL
  DROP PROCEDURE actividades.insertarDeporteDisponible;
GO

CREATE PROCEDURE actividades.insertarDeporteDisponible
...
CREATE PROCEDURE actividades.actualizarDeporteDisponible
  @idDeporte     INT,
  @descripcion   VARCHAR(20),
  @tipo          VARCHAR(10),
  @costoPorMes   DECIMAL(10,2)
...
IF OBJECT_ID('actividades.eliminarDeporteDisponible', 'P') IS NOT NULL
  DROP PROCEDURE actividades.eliminarDeporteDisponible;
GO

CREATE PROCEDURE actividades.eliminarDeporteDisponible
  @idDeporte INT
AS
BEGIN
  SET NOCOUNT ON;
...
BEGIN
  ...
  INTO @nombreSocio, @apellidoSocio
  FROM socios.socio
  WHERE idSocio = @idSocio
    AND categoriaSocio = @categoriaSocio;
...
--No haremos SP para actualizar un cobro. Una vez emitido, ya está. Tiene validez legal.
...
CREATE PROCEDURE pagos.insertarFacturaEmitida
  @idSocio                      INT,
  @categoriaSocio               INT,
...
BEGIN
  ...
  -- 2) Calcular porcentaje total de descuento vigente
  ...
  WHERE dv.idSocio = @idSocio
    AND dv.estadoDescuento = 1;

  -- 3) Calcular importeTotal
  SET @importeTotalCalc = ROUND(@importeBruto * (1 - @porcTotalDescuento/100.0), 2);

  -- 4) Fechas de vencimiento: a 5 y 10 días de hoy
...
--Misma consideración para el SP de actualización de factura.

CREATE PROCEDURE pagos.eliminarFacturaEmitida
  @idFactura INT
AS
BEGIN
  SET NOCOUNT ON;
  ...
  -- Verificar existencia
  IF NOT EXISTS (
    SELECT 1 FROM pagos.facturaEmitida WHERE idFactura = @idFactura
  )
  BEGIN
    RAISERROR('Factura %d no encontrada.',16,1,@idFactura);
    RETURN;
  END
...
BEGIN
  ...
  BEGIN
    RAISERROR('El monto a reembolsar debe ser mayor que cero.',16,1);
    RETURN;
  END

  -- 2) Validar CUIL destinatario
  IF @cuilDestinatario <= 0
  BEGIN
    RAISERROR('El CUIL del destinatario debe ser un número positivo.',16,1);
    RETURN;
  END

  -- 3) Verificar cobro original existe
  IF NOT EXISTS (
    SELECT 1 FROM pagos.cobroFactura WHERE idCobro = @idCobroOriginal
  )
  BEGIN
    RAISERROR('No se encontró el cobro original con ID %d.',16,1,@idCobroOriginal);
    RETURN;
  END

  -- 4) Verificar socio destinatario existe
  IF NOT EXISTS (
    SELECT 1 FROM socios.socio WHERE idSocio = @idSocioDestinatario
  )
  BEGIN
    RAISERROR('No se encontró el socio destinatario con ID %d.',16,1,@idSocioDestinatario);
    RETURN;
  END
...
BEGIN
  ...
  BEGIN
    RAISERROR('Reembolso %d / cobro %d no encontrado.',16,1,@idFacturaReembolso,@idCobroOriginal);
    RETURN;
  END
